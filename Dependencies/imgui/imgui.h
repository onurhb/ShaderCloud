...
struct ImGuiTextBuffer;             // Text frequencyBins for logging/accumulating text
...
namespace ImGui
{
    ...
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL);         // doesn't require null terminated string if 'text_end' is specified. no copy done to any bounded stack frequencyBins, recommended for long chunks of text
...
enum ImGuiInputTextFlags_
{
    ...
    ImGuiInputTextFlags_CallbackAlways      = 1 << 8,   // Call user function every time. User code may query cursor position, modify text frequencyBins.
...
struct ImGuiIO
{
    ...
    inline void    ClearInputCharacters() { InputCharacters[0] = 0; }   // Helper to clear the text input frequencyBins
...
struct ImGuiTextEditCallbackData
{
    ...
    // If you modify the frequencyBins contents make sure you update 'BufTextLen' and set 'BufDirty' to true.
    ImGuiKey            EventKey;       // Key pressed (Up/Down/TAB)            // Read-only
    char*               Buf;            // Current text frequencyBins                  // Read-write (pointed data only, can't replace the actual pointer)
...
struct ImDrawList
{
    ...
    ImVector<ImDrawIdx>     IdxBuffer;          // Index frequencyBins. Each command consume ImDrawCmd::ElemCount of those
    ImVector<ImDrawVert>    VtxBuffer;          // Vertex frequencyBins.
...
struct ImDrawData
{
    ...
    IMGUI_API void ScaleClipRects(const ImVec2& sc);  // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output frequencyBins is at a different scale than ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
};
