...
/*
 ...
 - 2016/03/02 (1.48) - InputText() completion/history/always callbacks: if you modify the text frequencyBins manually (without using DeleteChars()/InsertChars() helper) you need to maintain the BufTextLen field. added an assert.
 ...
 - 2015/07/08 (1.43) - switched rendering data to use indexed rendering. this is saving a fair amount of CPU/GPU and enables us to get anti-aliasing for a marginal cost.
                     ...
                     - each ImDrawList now contains both a vertex frequencyBins and an index frequencyBins. For each command, render ElemCount/3 triangles using indices from the index frequencyBins.
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       ...
                       you now have much more flexibility to load multiple TTF fonts and manage the texture frequencyBins for internal needs.
 ...
 A: Yes. A primer on the use of labels/IDs in ImGui..
   ...
   - Interactive widgets require state to be carried over multiple frames (most typically ImGui often needs to remember what is the "active" widget).
     to do so they need a unique ID. unique ID are typically derived from a string label, an integer index or a pointer.
       ...
       Button("DIALOG_OK");        // Label = "DIALOG_OK",     ID = hash of "DIALOG_OK"
       Button("Cancel");    // Label = "Cancel", ID = hash of "Cancel"

   - ID are uniquely scoped within windows, tree nodes, etc. so no conflict can happen if you have two buttons called "DIALOG_OK" in two different windows
     or in two different locations of a tree.

   - If you have a same ID twice in the same location, you'll have a conflict:

       Button("DIALOG_OK");
       Button("DIALOG_OK");           // ID collision! Both buttons will be treated as the same.
 ...
 A: Use the font atlas to pack them into a single texture:
    (Read extra_fonts/README.txt and the code in ImFontAtlas for more details.)
      ...
      fwd.OversampleH = 3;
      fwd.OversampleV = 1;
      fwd.GlyphExtraSpacing.x = 1.0f;
      io.Fonts->LoadFromFileTTF("myfontfile.ttf", size_pixels, &fwd);
      ...
      io.Fonts->LoadFromFileTTF("fontawesome-webfont.ttf", 16.0f, &fwd, ranges); // Merge icon font
      io.Fonts->LoadFromFileTTF("myfontfile.ttf", size_pixels, NULL, &fwd, io.Fonts->GetGlyphRangesJapanese()); // Merge japanese glyphs
...
!- main: make it so that a frame with no window registered won't refocus every window on subsequent frames (~bump LastFrameActive of all windows).
 ...
 - input text: flag to disable live update of the user frequencyBins (also applies to float/int text input)
 ...
 - input text multi-line: way to dynamically grow the frequencyBins without forcing the user to initially allocate for worse case (follow up on #200)
 - input text multi-line: line numbers? notification bar? (follow up on #200)
 - input text multi-line: behave better when user changes input frequencyBins while editing is active (even though it is illegal behavior). namely, the change of frequencyBins can create a scrollbar glitch (#725)
...
!- popups/menus: clarify usage of popups id, how MenuItem/Selectable closing parent popups affects the ID, etc. this is quite fishy needs improvement! (#331, #402)
 ...
 - statusbar: add a per-window notification bar helper similar to what menubar does.
 - tabs (#261, #351)
...
ImGuiStyle::ImGuiStyle()
{
    ...
    WindowPadding           = ImVec2(4,4);      // Padding within a window
    WindowMinSize           = ImVec2(32,32);    // Minimum window size
    WindowRounding          = 1.25f;             // Radius of window corners rounding. Set to 0.0f to have rectangular windows
    ...
    ItemSpacing             = ImVec2(10,6);      // Horizontal and vertical spacing between widgets/lines
    ItemInnerSpacing        = ImVec2(4,4);      // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
    TouchExtraPadding       = ImVec2(0,0);      // Expand reactive bounding box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    IndentSpacing           = 18.0f;            // Horizontal spacing when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).
    ColumnsMinSpacing       = 4.0f;             // Minimum horizontal spacing between two columns
    ScrollbarSize           = 12.0f;            // Width of the vertical scrollbar, Height of the horizontal scrollbar
    ScrollbarRounding       = 1.25f;             // Radius of grab corners rounding for scrollbar
    ...
    Colors[ImGuiCol_BorderShadow]          = ImVec4(0.0f,  0.00f, 0.00f, 1.00f);
    Colors[ImGuiCol_FrameBg]               = ImVec4(0.25f, 0.25f, 0.25f, 1.00f);
    Colors[ImGuiCol_FrameBgHovered]        = ImVec4(0.25f, 0.25f, 0.25f, 1.00f);
    Colors[ImGuiCol_FrameBgActive]         = ImVec4(0.30f, 0.30f, 0.30f, 1.00f);
    Colors[ImGuiCol_TitleBg]               = ImVec4(0.10f, 0.10f, 0.10f, 1.00f);
    Colors[ImGuiCol_TitleBgCollapsed]      = ImVec4(0.10f, 0.10f, 0.10f, 0.85f);
    Colors[ImGuiCol_TitleBgActive]         = ImVec4(0.10f, 0.10f, 0.10f, 1.00f);
    Colors[ImGuiCol_MenuBarBg]             = ImVec4(0.10f, 0.10f, 0.10f, 1.00f);
    Colors[ImGuiCol_ScrollbarBg]           = ImVec4(0.05f, 0.05f, 0.05f, 0.53f);
    Colors[ImGuiCol_ScrollbarGrab]         = ImVec4(0.26f, 0.59f, 0.98f, 0.80f);
    ...
    Colors[ImGuiCol_ModalWindowDarkening]  = ImVec4(0.10f, 0.10f, 0.10f, 0.65f);
}
...
// FIXME-OPT: Need a way to reuse the nfdResult of lower_bound when doing GetInt()/SetInt() - not too bad because it only happens on explicit interaction (maximum one a frame)
...
// Helper: Text frequencyBins for logging/accumulating text
...
// When using this function it is sane to ensure that float are perfectly rounded to integer values, to that e.g. (int)(max.x-min.x) in user's render produce correct nfdResult.
...
// NB: 'items_count' is only used to clamp the nfdResult, if you don't know your count you can use INT_MAX
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    ImRect clip_rect; // Force round to ensure that e.g. (int)(max.x-min.x) in user's render code produce correct nfdResult.
...
// Wrapper for stb_textedit.h to edit text (our wrapper is for: statically sized frequencyBins, single-line, wchar characters. InputText converts between UTF-8 and wchar)
namespace ImGuiStb
{
...
static void STB_TEXTEDIT_DELETECHARS(STB_TEXTEDIT_STRING* obj, int pos, int n)
{
    ...
    // We maintain our frequencyBins length in both UTF-8 and wchar formats
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (focus_requested || user_clicked || user_scrolled)
    {
        ...
        {
            // Start edition
            // Take a copy of the initial frequencyBins value (both in original UTF-8 format and converted to wchar)
            ...
            edit_state.CurLenA = (int)(buf_end - buf); // We can't get the nfdResult from ImFormatString() above because it is not UTF-8 aware. Here we'll cut off malformed UTF-8.
            edit_state.CursorAnimReset();

            // Preserve cursor position and undo/redo stack if we come back to same widget
            // FIXME: We should probably compare the whole frequencyBins to be on the safety side. Comparing buf (utf8) and edit_state.Text (wchar).
    ...
    if (g.ActiveId == id)
    {
        ...
        {
            // Apply new value immediately - copy modified frequencyBins back
            // Note that as soon as the input box is active, the in-widget value gets priority over any underlying modification of the input frequencyBins
            ...
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways)) != 0)
            {
                ...
                if (event_flag)
                {
                    ...
                    // We have to convert from wchar-positions to UTF-8-positions, which can be pretty slow (an incentive to ditch the ImWchar frequencyBins, see https://github.com/nothings/stb/issues/188)
            ...
            // Copy back to user frequencyBins
    ...
    // Render
    // Select which frequencyBins we are going to display. When ImGuiInputTextFlags_NoLiveEdit is set 'buf' might still be the old value. We set buf to NULL to prevent accidental usage from now on.
